...
export function getLastCheckpointLog(team: number, checkpointLogs: EntityState<CheckpointLog, number>) : CheckpointLog | null {
...
export function getPositionDescription(tracker: Tracker, fiches: EntityState<Fiche, number>, tochten: EntityState<Tocht, number>, weides: EntityState<Weide, number>) : string {
    ...
    if (tracker.tocht !== null) {
        ...
        const closest_fiches = Object.values(fiches.entities)
            .map((fiche) => ({
...
export function getCheckpointLog(team: number, fiche: number, checkpointLogs: EntityState<CheckpointLog, number>) {
